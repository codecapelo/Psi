...
const variantClasses: Record<ButtonVariant, string> = {
  ...
  ai: "bg-gradient-to-r from-violet-600 to-brand-600 text-white hover:from-violet-700 hover:to-brand-700",
};
...
// --------------------------------------------------------------------------
const fieldBase =
  "w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 disabled:opacity-60 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100";
...
export function Tooltip({ text }: { text: string }) {
  return (
    <span className="group relative inline-flex">
      <HelpCircle className="h-3.5 w-3.5 text-slate-400 hover:text-brand-500" />
      <span className="pointer-events-none absolute bottom-full left-1/2 z-30 mb-1 hidden w-64 -translate-x-1/2 rounded-md bg-slate-800 px-2.5 py-1.5 text-xs leading-snug text-white shadow-lg group-hover:block dark:bg-slate-700">
        {text}
      </span>
    </span>
  );
...
export function Modal({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "max-h-[90vh] w-full overflow-y-auto rounded-xl bg-white shadow-xl dark:bg-slate-900",
          sizes[size],
        )}
